...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    input_layer = X
    layer1, layer1_cache = affine_relu_forward(input_layer, W1, b1)
    layer2, layer2_cache = affine_forward(layer1, W2, b2)
    scores = layer2
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # exps = np.exp(scores)
    # row_exp_sums = np.sum(exps, axis=1)
    # loss += np.sum(-scores[range(N), y] + np.log(row_exp_sums))
    # loss /= N
    loss += 0.5 * reg * (np.sum(W1*W1) + np.sum(W2*W2))
    #
    # dout = (exps.T / row_exp_sums).T
    # ys = np.zeros_like(dout)
    # ys[range(N), y] = 1
    # dout -= ys
    # dout /= N

    layer2_dx, grads['W2'], grads['b2'] = affine_backward(dout, layer2_cache)
    layer1_dx, grads['W1'], grads['b1'] = affine_relu_backward(layer2_dx, layer1_cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # affine
    scores, cache[self.num_layers-1] = affine_forward(X, \
    ...
    dout, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dout, cache[self.num_layers-1])
    ...
    for i in reversed(xrange(1, self.num_layers)):
      ...
      if self.use_batchnorm:
        dout, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = \
          affine_batchnorm_relu_backward(dout, cache[i-1]) # cache index from 0
      else:
        dout, grads['W'+str(i)], grads['b'+str(i)] = affine_relu_backward(dout, cache[i-1]) # cache index from 0
